...
### 2.1 Window、Scene 和多窗口

iOS 13 之后，`UIWindow` 不再只是 App 级别的唯一窗口。一个 App 进程可以有多个 `UIWindowScene`，每个 Scene 可以有自己的 window。iPhone 上大多数业务仍然只有一个 Scene，但 iPad 多窗口、外接显示器、Stage Manager 都会让这个模型变得重要。
...
`keyWindow` 在多 Scene 下可能不是当前页面所在的 window。更稳妥的方式是从当前 View 或 ViewController 获取：
...
这类代码常用于全局 Toast、弹窗、路由入口。但全局 UI 能少用就少用，过度依赖 window 会让页面层级和生命周期变得难以控制。

### 2.2 Window Level 与全局浮层

`UIWindow` 有 `windowLevel`。系统键盘、状态栏、Alert、普通业务窗口都有不同层级。业务如果创建悬浮窗，要明确层级和生命周期。

```objc
UIWindow *overlayWindow = [[UIWindow alloc] initWithWindowScene:windowScene];
overlayWindow.windowLevel = UIWindowLevelAlert + 1;
overlayWindow.rootViewController = [[YWOverlayViewController alloc] init];
overlayWindow.hidden = NO;
self.overlayWindow = overlayWindow;
```

这里必须强持有 `overlayWindow`。如果只是局部变量，方法结束后就会释放，浮层会消失。
...
### 3.1 ViewController 生命周期放什么

`UIViewController` 最容易写乱的原因，是所有代码都能放进去，但不是所有代码都应该放进去。
...
- `init`：保存页面必需参数，不访问 `self.view`。
- `loadView`：纯代码创建根 View；如果不重写，系统会创建默认 View 或加载 Storyboard/Xib。
- `viewDidLoad`：一次性 UI 构建、约束、事件绑定。
- `viewWillAppear:`：页面每次出现前的轻量刷新。
- `viewDidAppear:`：曝光埋点、动画启动、依赖页面可见的操作。
- `viewWillDisappear:`：暂停动画、取消不需要的请求、停止计时器。
- `dealloc`：确认通知、KVO、Timer、DisplayLink、Block 是否释放。

常见错误是把接口请求无脑放进 `viewWillAppear:`，导致页面每次返回都重复请求。应该区分首次加载和每次刷新：
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self refreshVisibleStateIfNeeded];
}
```

### 3.2 ViewController 容器关系

`UINavigationController`、`UITabBarController`、`UIPageViewController` 都是容器控制器。自定义容器时，不能只把子控制器的 view 加上去，还要维护父子关系。

```objc
- (void)embedChildViewController:(UIViewController *)child inView:(UIView *)containerView {
...
- (void)removeEmbeddedChildViewController:(UIViewController *)child {
...
如果缺少这些调用，子控制器的生命周期转发、旋转、Safe Area、状态栏控制都可能异常。

### 3.3 控制器释放排查

页面返回后不释放，通常不是 UIKit 问题，而是引用链没断。
...
- Block 是否强捕获 `self`。
- `NSTimer` / `CADisplayLink` 是否 invalidate。
- 通知、KVO 是否解除。
- 子控制器是否正确移除。
- 单例、Manager、缓存数组是否持有页面。
...
`__strong` 提升不是形式代码。它保证 weak 读取成功后，Block 执行期间对象不被释放。
...
### 4.1 Frame、Bounds、Center 的区别

`frame`、`bounds`、`center` 是 `UIView` 坐标体系的基础。

- `frame`：View 在父视图坐标系中的位置和尺寸。
- `bounds`：View 自己坐标系中的位置和尺寸。
- `center`：View 在父视图坐标系中的中心点。
...
`bounds.origin` 平时多为 `{0, 0}`，但在 `UIScrollView` 中滚动时会发生变化。ScrollView 本质上就是通过改变 bounds 的 origin 来显示内容的不同区域。

### 4.2 View 层级与事件命中

View 不只是显示树，也是事件命中树。触摸事件会从 window 开始，通过 `hitTest:withEvent:` 找到最合适的 View。

```mermaid
flowchart TD
    A["UIWindow receives touch"] --> B["hitTest:withEvent:"]
    B --> C["pointInside:withEvent:"]
    C --> D["frontmost eligible subview"]
    D --> E["responder handles event"]
```

影响命中的因素：

- `hidden = YES` 不参与命中。
- `userInteractionEnabled = NO` 不接收事件。
- `alpha` 太低时不参与命中。
- 点不在 `bounds` 内通常不命中。
- 后添加的 subview 视觉上更靠前，也更先参与命中。

扩大点击区域时，不要随便加透明 View。可以重写 `pointInside:withEvent:`：

```objc
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
    CGRect touchBounds = CGRectInset(self.bounds, -12.0, -12.0);
    return CGRectContainsPoint(touchBounds, point);
}
```

### 4.3 setNeedsLayout、layoutIfNeeded、drawRect

布局和绘制是两件事。

- `setNeedsLayout`：标记需要重新布局，稍后系统调用 `layoutSubviews`。
- `layoutIfNeeded`：如果已经标记需要布局，立即布局。
- `setNeedsDisplay`：标记需要重绘，稍后系统调用 `drawRect:`。

动画中常见写法：

```objc
self.heightConstraint.constant = expanded ? 180.0 : 64.0;
[UIView animateWithDuration:0.25 animations:^{
    [self.view layoutIfNeeded];
}];
```

`drawRect:` 只适合自定义绘制。普通业务 UI 不要为了画边框、圆角就重写 `drawRect:`，Layer 属性通常更合适。
...
`UILabel` 的难点不是设置文字，而是动态内容下的高度计算和截断策略。
...
Auto Layout 下，多行 Label 要有明确宽度，系统才能推导高度。TableView Cell 中要让约束从 contentView 顶部贯通到底部。
...
横向两个 Label 争抢空间时，要用 Hugging 和 Compression Resistance 表达优先级：
...
这里表示价格不应该被压缩，标题可以更早截断。

### 5.2 富文本和复用

富文本要注意复用残留。如果某个 Cell 上一次设置了红色高亮，下一次只设置普通 `text`，可能仍有状态混淆。推荐在渲染方法里完整覆盖。

```objc
- (void)renderWithTitle:(NSString *)title keyword:(nullable NSString *)keyword {
...
### 5.3 国际化和 Dynamic Type

高级工程里不能假设中文、英文、阿拉伯语都占同样宽度，也不能假设用户一定用默认字体大小。面向系统字体变化时，应考虑 Dynamic Type：
...
`UIButton` 继承自 `UIControl`。Target-Action 的本质是：控件在特定事件发生时，向目标对象发送 selector。

常用事件：

- `UIControlEventTouchUpInside`：手指在按钮内抬起，最常用。
- `UIControlEventTouchDown`：按下。
- `UIControlEventValueChanged`：值变化，常见于 Switch、Slider。
- `UIControlEventEditingChanged`：输入变化，常见于 TextField。

不要把复杂业务都写在按钮点击方法里，按钮事件应该尽快转交业务方法。
...
这样后续键盘 return、导航栏按钮、自动提交都可以复用 `submitIfNeeded`。
...
- (void)submitIfNeeded {
    ...
    __weak typeof(self) weakSelf = self;
    [self.service submitWithCompletion:^(NSError * _Nullable error) {
        __strong typeof(weakSelf) self = weakSelf;
        if (!self) {
            return;
        }
...
只禁用按钮不够，业务层也要有 `isSubmitting` 保护。否则多个入口触发同一提交逻辑时仍可能重复请求。

### 6.3 Configuration 与状态样式

现代 UIKit 可以用 `UIButtonConfiguration` 管理按钮样式。老 Objective-C 项目仍常用传统 API，但要理解按钮状态样式应该集中配置，而不是散落在点击回调里。

```objc
if (@available(iOS 15.0, *)) {
    UIButtonConfiguration *configuration = [UIButtonConfiguration filledButtonConfiguration];
    configuration.title = @"提交";
    configuration.cornerStyle = UIButtonConfigurationCornerStyleMedium;
    self.submitButton.configuration = configuration;
}
```

不管使用新旧 API，按钮都要覆盖 normal、disabled、highlighted 等关键状态，避免“能点但看不出来”或“不能点但像能点”。
...
`UIImageView` 只负责显示图片，但图片显示经常带来性能问题。大图从网络下载后，如果在主线程解码，滚动时会卡顿。

常见策略：

- 下载在后台。
- 解码在后台。
- 主线程只设置已经准备好的 `UIImage`。
- 列表中使用缩略图，不直接显示原图。
- 内存警告时清理可恢复缓存。
...
这段代码表达的是“后台预解码”的思路。真实项目通常交给图片库或统一 ImageLoader 处理。

### 7.2 imageNamed 与 imageWithContentsOfFile

`imageNamed:` 会缓存图片，适合 App bundle 内的小图标、常用资源图。

```objc
UIImage *icon = [UIImage imageNamed:@"tab_home"];
```

大图、临时图、只用一次的图片，不应无脑用 `imageNamed:`。可以使用文件路径加载：

```objc
UIImage *image = [UIImage imageWithContentsOfFile:filePath];
```

判断标准不是“哪个 API 更方便”，而是这张图是否应该长期缓存。
...
图片展示层应只接受目标 URL 或图片对象，下载、缓存、取消请求应由 ImageLoader 管理。
...
`UITextField` 获取焦点后会成为 first responder，系统键盘随之出现。
...
键盘不是 ViewController 的子视图，不能通过改输入框 frame 解决所有问题。更稳定的方式是监听键盘 frame 变化，调整底部约束或 ScrollView inset。
...
使用 `WillChangeFrame` 比只监听 show/hide 更完整，因为键盘高度可能因为输入法切换、候选栏、横竖屏变化而改变。

### 8.2 输入验证和组合输入

限制长度时要注意中文输入法的 marked text。用户正在拼音组合时，直接截断会破坏输入体验。
...
金额输入、手机号输入、验证码输入都应该有明确规则。不要只依赖键盘类型，因为外接键盘、粘贴、自动填充都可能绕过键盘限制。
...
`textContentType` 会影响系统自动填充能力。登录、验证码、邮箱、姓名、地址等输入框应按语义配置。

敏感输入不要写日志：

```objc
NSLog(@"password: %@", self.passwordField.text); // Bad
```

### 8.4 UITextField 掌握标准

掌握 `UITextField`，至少要能做到：

- 能解释 first responder 和键盘出现的关系。
- 能处理键盘遮挡，不写死键盘高度。
- 能使用 delegate 和 editing changed 事件。
- 能处理中文 marked text。
- 能限制手机号、金额、验证码等输入。
- 能配置 `secureTextEntry`、`textContentType`、`keyboardType`、`returnKeyType`。
- 能避免敏感输入进入日志、埋点和崩溃上报。
...
`UINavigationController` 持有一个 `viewControllers` 数组。push 是把新控制器压入数组尾部，pop 是移除尾部控制器。

```objc
NSArray<UIViewController *> *stack = self.navigationController.viewControllers;
NSLog(@"navigation stack: %@", stack);
```

这解释了几个现象：

- Root 控制器不会被 pop 掉。
- push 后旧页面通常不释放，因为它仍在栈里。
- pop 后被移除的控制器如果没有其他强引用，才会释放。
- 直接修改 `viewControllers` 可以重构返回路径。

页面返回后不释放时，要先判断它是不是仍在导航栈里。

```objc
BOOL stillInStack = [self.navigationController.viewControllers containsObject:self];
```

如果仍在栈里，不释放是正常的；如果已经不在栈里但还不释放，再查 Block、Timer、通知、单例引用。

### 12.2 Push、Pop 与 Present 的边界

`push` 表示同一个业务路径里的层级深入，比如列表到详情。`present` 表示临时流程或独立任务，比如登录、扫码、分享、选择器。

判断规则：

- 用户完成后应该回到原页面层级：优先 present。
- 用户进入信息架构的下一层：优先 push。
- 模态流程内部还需要继续 push：present 一个 NavigationController。
...
不要用 push 登录页再手动隐藏返回按钮，这会污染业务导航栈。登录是全局状态流程，更适合 present 或根控制器切换。

### 12.3 侧滑返回和自定义返回按钮

系统侧滑返回由 `interactivePopGestureRecognizer` 管理。自定义返回按钮时，容易把侧滑返回弄失效。
...
如果自定义返回后要保留侧滑，需要谨慎设置手势代理。不要在不了解当前导航栈状态时简单写：

```objc
self.navigationController.interactivePopGestureRecognizer.delegate = nil;
```

更稳定的做法是集中封装导航控制器，判断 root 页面不允许侧滑，非 root 页面允许侧滑，避免根页面侧滑导致手势状态异常。
...
导航控制器管理页面栈，不应该承载业务逻辑。业务是否能进入某页面，应由 Router、权限服务或业务层决定。导航控制器只负责执行 push/pop/set stack。

推荐的路由入口：

```objc
- (void)openArticleDetailWithArticleId:(NSString *)articleId from:(UIViewController *)source {
    if (articleId.length == 0) {
        return;
    }

    YWArticleDetailViewController *detail = [[YWArticleDetailViewController alloc] initWithArticleId:articleId];
    [source.navigationController pushViewController:detail animated:YES];
}
```

页面不需要知道详情页的复杂创建过程，路由层也不应该直接处理网络请求。
...
### 13.1 navigationItem 和 navigationBar 的关系

`UINavigationBar` 是导航控制器持有的顶部栏；每个页面通过自己的 `navigationItem` 提供标题、左右按钮、返回项。导航控制器会根据栈顶控制器的 `navigationItem` 更新导航栏显示。

```mermaid
flowchart TD
    A["UINavigationController"] --> B["UINavigationBar"]
    A --> C["Top UIViewController"]
    C --> D["navigationItem"]
    D --> B
```
...
- 不要在每个页面直接创建一个导航栏盖在顶部。
- 页面级标题、按钮应该设置 `self.navigationItem`。
- 全局导航栏样式可以配置 `navigationBar` 或 appearance。

### 13.2 Appearance 与全局样式

iOS 13 后推荐使用 `UINavigationBarAppearance` 配置导航栏。
...
`standardAppearance` 和 `scrollEdgeAppearance` 不一致时，页面滚动到顶部可能出现导航栏突然透明或颜色变化。
...
大标题依赖滚动视图的内容位置。TableView / CollectionView 顶部 inset、`contentInsetAdjustmentBehavior`、透明导航栏都会影响大标题表现。

如果页面顶部有自定义 Header，出现大标题抖动或遮挡，要检查：

- ScrollView 是否约束到 Safe Area。
- 是否关闭了自动 inset。
- `scrollEdgeAppearance` 是否配置。
- Header 是否改变了 contentInset 或 contentOffset。

### 13.4 顶部按钮的工程规范

导航栏按钮属于页面操作入口，不应该放太多。多个按钮时要考虑触达成本和信息架构。

按钮事件不要直接写复杂逻辑：

```objc
- (void)shareButtonTapped:(UIBarButtonItem *)sender {
    [self shareCurrentArticle];
}
...
- (void)shareCurrentArticle {
    ...
    [self.shareService shareArticle:self.article from:self];
}
```

这样分享能力可以被更多入口复用，不绑定某个 bar button。
...
### 14.1 Modal Presentation Style

iOS 13 之后，默认模态样式不再总是全屏，很多页面会以卡片形式展示。常见样式：

- `UIModalPresentationFullScreen`：全屏。
- `UIModalPresentationPageSheet`：页卡样式。
- `UIModalPresentationFormSheet`：表单样式，iPad 常见。
- `UIModalPresentationOverFullScreen`：覆盖在当前页面上，底层页面不移除。
- `UIModalPresentationPopover`：iPad 弹出气泡。
...
半屏选择器、筛选器、分享前确认，可以用 sheet 风格。

### 14.2 presentationController 与下拉关闭

卡片模态默认可能允许用户下拉关闭。某些流程不能被随意关闭，比如支付、强制登录、资料未保存页面。
...
还可以通过 `UIAdaptivePresentationControllerDelegate` 感知用户尝试关闭：
...
`dismissViewControllerAnimated:` 可以由 presented 控制器自己调用，也可以由 presenting 控制器调用。

```objc
[self dismissViewControllerAnimated:YES completion:nil];
```

如果模态页包了一层 NavigationController，内部子页面调用 dismiss 会关闭整个模态导航流程。

工程中常见错误：

- present 多层后不知道谁负责 dismiss。
- 支付/登录流程中途被下拉关闭，状态没处理。
- present 时当前页面已经在 presenting，导致警告或展示失败。
- 在 `viewDidLoad` 里立即 present，页面层级还未进入 window。

需要延迟到页面可见后展示时，放到 `viewDidAppear:` 并加状态保护。
...
UITabBarController
  ...
  ├─ UINavigationController(Message)
...
后者会让多个 Tab 共用一条导航栈，Tab 内返回路径容易混乱。每个一级模块应有自己的导航栈。

### 15.2 Tab 切换与页面生命周期

Tab 页面通常常驻。切换 Tab 时，旧页面不释放，只是从可见状态变成不可见状态。

这带来几个工程影响：

- 首页切到“我的”后，首页控制器通常还在内存里。
- `viewDidLoad` 不会因为切回来再次执行。
- `viewWillAppear:` 可能因为切换 Tab 多次触发。
- 音视频、计时器、定位等资源要根据可见性暂停。

不要用 `dealloc` 处理 Tab 切换资源暂停，因为 Tab 页面不会释放。

### 15.3 shouldSelect 与登录拦截
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController
shouldSelectViewController:(UIViewController *)viewController {
    if ([self isProfileNavigationController:viewController] && !self.session.isLoggedIn) {
        [self presentLoginFrom:tabBarController];
        return NO;
    }
...
这适合“我的”“消息”等需要登录的一级 Tab。不要先切过去再弹登录，否则用户会看到未登录页面闪一下。

### 15.4 双击 Tab 回到顶部

很多 App 支持点击当前 Tab 回到顶部或刷新。需要记录当前选中的控制器：

```objc
- (void)tabBarController:(UITabBarController *)tabBarController
 didSelectViewController:(UIViewController *)viewController {
    if (viewController == self.lastSelectedViewController) {
        [self scrollToTopForViewController:viewController];
    }
...
实现时不要让 TabBarController 直接知道每个页面内部 ScrollView。可以定义协议：

```objc
@protocol YWTabRefreshable <NSObject>
- (void)scrollToTopOrRefresh;
@end
```

页面自己实现协议，TabBarController 只调用能力。

### 15.5 Badge 与状态同步

Tab badge 通常来自未读数、通知数、购物车数量。

```objc
messageNav.tabBarItem.badgeValue = unreadCount > 0 ? @(unreadCount).stringValue : nil;
```

Badge 更新不要散落在各页面里。更合理的是由统一状态服务监听未读数变化，然后更新对应 Tab。

```objc
- (void)unreadCountDidChange:(NSInteger)count {
    self.messageNavigationController.tabBarItem.badgeValue = count > 0 ? @(count).stringValue : nil;
}
```

否则多个页面同时改 badge，很容易出现状态回退。
...
- 能解释 Scene、多 Window、key window、windowLevel 的适用边界。
- 能用 `UIViewController` 管理页面生命周期，并用初始化方法传递必要参数。
- 能正确维护自定义容器控制器的 parent-child 关系。
- 能排查 ViewController 不释放的引用链。
- 能理解 `UIView` 层级、frame/bounds、CALayer、圆角和阴影。
- 能解释 hit-testing、响应链、`setNeedsLayout`、`layoutIfNeeded`、`drawRect:` 的边界。
- 能熟练使用 `UILabel` 展示文本和富文本。
- 能处理动态文本、高度计算、富文本复用、字体放大和国际化。
- 能使用 `UIButton` 处理 Target-Action、状态和防重复点击。
- 能理解 `UIControl` 事件模型，并把按钮事件转交业务方法。
- 能使用 `UIImageView` 展示图片，并处理复用和异步加载问题。
- 能解释图片缓存、后台解码、大图内存和圆角阴影冲突。
- 能使用 `UITextField` 处理输入、键盘、Delegate 和输入限制。
- 能处理 first responder、键盘遮挡、中文 marked text、安全输入和自动填充。
...
- 能解释 push、pop、setViewControllers、自定义返回、侧滑返回的关系。
- 能用 `UINavigationBarAppearance` 处理导航栏样式、大标题和滚动联动。
- 能使用 `UITabBarController` 组织多个一级模块，并理解每个 Tab 独立导航栈。
- 能处理 Tab 登录拦截、双击回顶、badge 状态同步、Tab 页面常驻资源管理。
