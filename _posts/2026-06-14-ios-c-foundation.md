...
`unsigned char` 如果是 1 字节，最大值通常是 `255`。再加 1 后会回到 `0`。这就是溢出。写业务代码时很少直接用这么小的类型，但这个例子能说明“类型不是随便写的”。
...
更准确的说法是：函数通过指针参数产生副作用，修改了调用方传入地址上的数据。这和 `return` 返回值是两件事。
...
C / Objective-C 代码不是直接运行的。Xcode 点下 Build 后，每个 `.c`、`.m`、`.mm` 文件都会经过一条构建链路：预处理、编译、汇编、链接。
...
理解这条链路，不是为了手写编译器，而是为了能看懂 Xcode 报错：有些错误发生在编译阶段，有些错误发生在链接阶段，处理方式完全不同。

### 预处理

预处理发生在真正编译之前。它主要处理 `#include`、`#import`、`#define`、条件编译等内容。

```objective-c
#import "User.h"

#if DEBUG
#define LogInfo(msg) NSLog(@"%@", msg)
#else
#define LogInfo(msg)
#endif
```

预处理器不会理解业务逻辑，它做的是文本层面的展开和替换。

- `#import "User.h"` 会把 `User.h` 中的声明带进当前文件。
- `#define` 会在编译前进行宏替换。
- `#if DEBUG` 会决定某段代码是否参与后续编译。
...
这就是宏容易出错的原因：它不是函数调用，而是编译前的文本替换。

### 编译

编译阶段会检查语法、类型、函数调用是否匹配，并把源代码转换成更底层的中间表示。
...
这种类型不匹配的问题，通常会在编译阶段暴露。编译器会发现 `NSString *` 和 `NSInteger` 不是同一类东西。
...
当某个文件调用 `add(1, 2)` 时，编译器只需要先知道函数声明，就能判断参数数量、参数类型和返回值类型是否合理。此时不要求函数实现已经出现在当前文件里。

```c
int result = add(1, 2);
```

这解释了为什么 `.h` 文件可以只放声明。声明是给编译器看的，它让编译器知道“这个符号可以这样使用”。

### 汇编

汇编阶段会把编译结果转换成目标文件。目标文件通常是 `.o` 文件。它已经不是普通源代码，但也还不是最终 App。

可以把每个 `.m` 文件理解成先被单独处理成一个 `.o` 文件：

```text
User.m      -> User.o
MathTool.m  -> MathTool.o
AppDelegate.m -> AppDelegate.o
```
...
例如 `User.o` 里可能定义了 `User` 类相关符号，也可能依赖 `NSLog`、`UIView` 或其他文件里的函数。

### 链接

链接阶段会把多个目标文件和依赖库组合起来，生成最终可执行文件。这个阶段最关心的是“符号能不能对上”。

如果某个文件调用了 `add`，但整个工程里没有任何地方真正实现 `add`，链接器会报类似“找不到符号”的错误。
...
如果只有声明，没有任何 `.c` 文件提供实现，编译可能能过，但链接会失败。因为编译器只需要声明，链接器需要找到真正的实现。

相反，如果多个地方都定义了同一个全局符号，链接器也会报错。
...
这不是“看不见实现”，而是“实现太多了”。链接器不知道最终应该使用哪一个 `count`。

Objective-C 项目里常见的链接错误也属于这一类问题：

- 忘记把 `.m` 文件加入 Target，导致找不到类或函数实现。
- 引入了头文件，但没有链接对应的静态库或动态库。
- 第三方库重复集成，导致符号重复。
- C 函数或全局变量命名过于普通，和其他库撞名。
...
这种问题不能靠“头文件声明、源文件定义”自动消除，而是从设计上就不应该让两个模块暴露同名函数实现。应该改成更明确的名字：
